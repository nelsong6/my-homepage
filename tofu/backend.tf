...
resource "azurerm_cosmosdb_sql_role_assignment" "container_app_cosmos" {
  ...
  principal_id        = azurerm_container_app.homepage_api.identity[0].principal_id
...
resource "azurerm_dns_txt_record" "homepage_api_verification" {
  ...
  record {
    value = azurerm_container_app.homepage_api.custom_domain_verification_id
  }
}
...
resource "azurerm_dns_cname_record" "homepage_api" {
  ...
  resource_group_name = var.resource_group_name
  ttl                 = 3600
  record              = azurerm_container_app.homepage_api.ingress[0].fqdn
}
...
resource "azurerm_container_app_custom_domain" "homepage_api" {
  ...
  container_app_id = azurerm_container_app.homepage_api.id
